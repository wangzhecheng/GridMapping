...
region_list = ['Kampala_Ntinda', 'Kampala_Kololo', 'Nairobi_Highridge', 'Nairobi_Ngara', 'Lagos_Ikeja2']

benchmark_version = 'corrected' # manually curated by checking street views and remote sensing images
...
data_dir = 'data'
performance_dict_save_path = 'results/aggregated/africa_pole_perf_ori0.2_' + benchmark_version + '.pickle'
if_evaluate = True
...
def load_road_data(region):
    # load road data
    with open(join('data/road_info/africa', region,
    ...
    with open(join('data/road_info/africa', region, 'way_tag_dict_local.pickle'),
              'rb') as f:
...
def get_discretization_params(lat_min, lat_max, lon_min, lon_max, lat_min_way, lat_max_way, lon_min_way, lon_max_way, idx2sv):
    ...
    lat_s = min(lat_min, lat_min_way, np.min(np.array(idx2sv)[:, 0])) - 100 * dlat0
    lat_n = max(lat_max, lat_max_way, np.max(np.array(idx2sv)[:, 0])) + 100 * dlat0
    lon_w = min(lon_min, lon_min_way, np.min(np.array(idx2sv)[:, 1])) - 100 * dlon0
    lon_e = max(lon_max, lon_max_way, np.max(np.array(idx2sv)[:, 1])) + 100 * dlon0
